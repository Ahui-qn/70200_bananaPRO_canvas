...
### 需要重启前端的情况
- 修改 `frontend/src/**/*.tsx` 或 `frontend/src/**/*.ts` 文件（Vite 通常会热更新，但复杂改动可能需要重启）
...
### 执行方式
使用 `controlBashProcess` 工具的 `start` action 来启动开发服务器：
- 先用 `stop` action 停止已有进程（如果有）
- 再用 `start` action 启动新进程
