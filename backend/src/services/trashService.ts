...
class TrashService {
  ...
  async getTrashItems(): Promise<TrashContent> {
    ...
    `;
    const projectRows = await databaseManager.executeQuery(projectsSql);
    ...
    `;
    const imageRows = await databaseManager.executeQuery(imagesSql);
  ...
  async restoreProject(projectId: string): Promise<Project> {
    ...
    `;
    const result = await databaseManager.executeQuery(projectSql, [new Date(), projectId]);
    ...
    `;
    await databaseManager.executeQuery(imagesSql, [projectId]);
  ...
  async restoreImage(imageId: string): Promise<void> {
    ...
    const imageSql = `
      ...
      WHERE id = ? AND is_deleted = TRUE
    `;
    const imageRows = await databaseManager.executeQuery(imageSql, [imageId]);
    ...
    if (projectId) {
      const projectSql = `
        ...
        WHERE id = ?
      `;
      const projectRows = await databaseManager.executeQuery(projectSql, [projectId]);
    ...
    const restoreSql = `
      ...
      WHERE id = ?
    `;
    await databaseManager.executeQuery(restoreSql, [targetProjectId, imageId]);
  }
    ...
    if (ossKeysToDelete.length > 0) {
      try {
        ...
        if (!storageManager.isConfigured()) {
          throw new Error('存储服务未配置或不可用');
        }

        // 逐个删除存储文件（避免使用批量删除，因为可能没有列表权限）
        let successCount = 0;
        let failedCount = 0;
        
        for (const ossKey of ossKeysToDelete) {
          try {
            const success = await storageManager.deleteObject(ossKey);
            if (success) {
              successCount++;
            } else {
              failedCount++;
              console.warn(`存储文件删除失败: ${ossKey}`);
            }
          } catch (error) {
            failedCount++;
            console.warn(`存储文件删除异常: ${ossKey}`, error);
          }
        }
        
        console.log(`存储文件删除完成: 成功 ${successCount}, 失败 ${failedCount}`);
        
        if (failedCount > 0) {
          console.warn(`部分存储文件删除失败，但继续执行数据库删除`);
        }
      } catch (error) {
        console.error(`删除项目存储文件失败: ${error}`);
        // 不抛出错误，允许继续删除数据库记录
        console.warn('存储文件删除失败，但继续执行数据库删除');
      }
    }

    // 硬删除图片记录
    const deleteImagesSql = `DELETE FROM ${TABLE_NAMES.IMAGES} WHERE project_id = ?`;
    await databaseManager.executeQuery(deleteImagesSql, [projectId]);

    // 硬删除项目记录
    const deleteProjectSql = `DELETE FROM ${TABLE_NAMES.PROJECTS} WHERE id = ?`;
    await databaseManager.executeQuery(deleteProjectSql, [projectId]);
  }
  ...
  async hardDeleteImage(imageId: string): Promise<void> {
    ...
    if (ossKeysToDelete.length > 0) {
      try {
        ...
        console.error(`删除图片存储文件失败: ${error}`);
        // 存储删除失败时，给用户明确提示
        throw new Error(`无法删除存储文件，请检查网络连接或联系管理员。错误: ${error instanceof Error ? error.message : String(error)}`);
      }
    }

    // 硬删除图片记录
    const deleteSql = `DELETE FROM ${TABLE_NAMES.IMAGES} WHERE id = ?`;
    await databaseManager.executeQuery(deleteSql, [imageId]);
  }
        ...
        if (storageManager.isConfigured()) {
          ...
          for (const ossKey of ossKeysToDelete) {
            try {
              ...
              failedCount++;
              console.warn(`删除存储文件失败: ${ossKey}`, error);
            }
          }
          
          if (failedCount > 0) {
            ossErrors.push(`部分存储文件删除失败: 成功 ${successCount}, 失败 ${failedCount}`);
          }
        } else {
          ossErrors.push('存储服务未配置，跳过存储文件删除');
        }
      } catch (error) {
        ossErrors.push(`存储文件删除失败: ${error instanceof Error ? error.message : String(error)}`);
      }
    }

    // 硬删除所有已删除的图片
    const deleteImagesSql = `DELETE FROM ${TABLE_NAMES.IMAGES} WHERE is_deleted = TRUE`;
    const imagesResult = await databaseManager.executeQuery(deleteImagesSql);

    // 硬删除所有已删除的项目
    const deleteProjectsSql = `DELETE FROM ${TABLE_NAMES.PROJECTS} WHERE is_deleted = TRUE`;
    const projectsResult = await databaseManager.executeQuery(deleteProjectsSql);
  ...
  async softDeleteImage(imageId: string, deletedBy: string): Promise<void> {
    ...
    `;
    const result = await databaseManager.executeQuery(sql, [now, deletedBy, imageId]);
